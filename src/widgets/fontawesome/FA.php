...
namespace bizley\podium\client\widgets\fontawesome;

use bizley\podium\client\widgets\fontawesome\assets\FontAwesomeAsset;
use Yii;
use yii\base\Widget;
...
/**
 * Class FA
 * @package bizley\podium\client\widgets\fontawesome
 *
...
class FA extends Widget
{
    ...
    public static function icon(string $icon, array $config = []): ?string
    {
        try {
            ...
            Yii::error(['fa.widget.error', $exc->getMessage(), $exc->getTraceAsString()], 'podium');
            return '';
        }
    }
